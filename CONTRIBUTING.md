...
This document serves as a guide for contributing to the OpenShift components/repositories that the OpenShift Control Plane group is responsible for maintaining.

This document is explicitly for contributions to individual component repositories and not for high-level feature proposals within OpenShift.

Feature proposals should follow the OpenShift Enhancement Proposal process outlined in https://github.com/openshift/enhancements/blob/master/dev-guide/feature-zero-to-hero.md#openshift-feature-development-zero-to-hero-guide. If you are looking for a review on an OpenShift Enhancement Proposal that involves changes to components maintained by the control plane group, please request a review in the [`#forum-ocp-workloads`](https://redhat.enterprise.slack.com/archives/CKJR6200N) Slack channel. Requests for review may be redirected to more appropriate and/or more focused channels for discussion.
...
- [Repo-specific guidelines](#repo-specific-guidelines) - Guidelines specific to this repository.
...
- When a function accepts or has access to a `context.Context`, pass it through to downstream calls. Never discard a context or substitute `context.Background()`/`context.TODO()`.
...
- Table-driven unit tests are preferred for testing multiple scenarios/inputs. For an example, see `pkg/operator/target_config_reconciler_test.go`.
...
You can use the `Cluster Bot` Slack App to create a cluster from a PR by sending it a message in the format of `launch ${OCP_VERSION},${PR_LINK} ${PLATFORM},${VARIANT}`. As an example, `launch 4.23,https://github.com/openshift/cluster-kube-descheduler-operator/pull/2027 aws,techpreview` would launch an OpenShift 4.23 cluster with the changes made in openshift/cluster-kube-descheduler-operator#2027 running on AWS with the TechPreviewNoUpgrade feature-set enabled. For more information on what `Cluster Bot` can do, you can send it a message saying `help` and it will respond with additional documentation on how it can be used.
...
The following guidelines are specific to the `cluster-kube-descheduler-operator` repository.

### Pre-submit Checks

```bash
make verify        # gofmt, go vet, version consistency
...
Dependencies are vendored. Always run both commands together when updating dependencies.

### Embedded Assets (bindata)

Asset manifests live in `bindata/assets/kube-descheduler/`. The `bindata/assets.go` file uses Go's `//go:embed` directive to embed them. To modify an asset, edit the YAML file directly — do not edit `assets.go`.

The CRD schema is extracted from `manifests/kube-descheduler-operator.crd.yaml` into `bindata/assets/kube-descheduler/crdschema.yaml` by `make regen-crd`. After API type changes, run `make generate` to regenerate the CRD and the extracted schema.

### Testing Conventions

**Unit tests** (`pkg/`):
- Co-located `*_test.go` files alongside source code
- Standard Go naming: `TestFunctionName` (e.g., `TestManageConfigMap`, `TestReconcile`)
- Table-driven tests preferred
- Test fixtures in `testdata/` directories (e.g., `pkg/operator/testdata/`, `pkg/softtainter/testdata/`)

**E2E tests** (`test/e2e/`):
- Uses the [OpenShift Tests Extension (OTE)](https://github.com/openshift-eng/openshift-tests-extension) framework
- Test functions are unexported camelCase in `operator.go` (e.g., `testPodDescheduling()`, `testSoftTainterController()`)
- Driven by `TestExtended()` in `operator_test.go` via OTE
- Helper/setup functions in `helpers.go`, `descheduler_utils.go`, `descheduler_verification.go`
- See [README.md](README.md#tests) for commands to build, run, and list test suites

**DAST tests** (`.tekton/integration-tests/`):
- Konflux-driven integration tests (not Go tests)
- Configs, pipelines, and scripts under `.tekton/integration-tests/`
