...
func TestAddOrUpdateSoftTaint(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			less := func(a, b v1.Taint) bool { return a.Key < b.Key }
			diffIgnoreOrder := cmp.Diff(tNode.Spec.Taints, tc.expectedTaints, cmpopts.SortSlices(less))
			if diffIgnoreOrder != "" {
				t.Errorf("Test %#v failed, node %v should different taints: %v", tc.description, tNode.Name, diffIgnoreOrder)
			}
