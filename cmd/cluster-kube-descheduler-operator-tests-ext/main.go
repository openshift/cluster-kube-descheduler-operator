/*
This command is used to run the Cluster Kube De-scheduler  Operator tests extension for OpenShift.
It registers the cluster-kube-descheduler-operator tests with the OpenShift Tests Extension framework
...
func newOperatorTestCommand(ctx context.Context) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "A binary used to run cluster-kube-descheduler-operator tests as part of OTE.",
